...
// console.log( "Name of file is :"+page );
document.querySelectorAll(".links li").forEach((el) => {
	el.addEventListener("click", () => {
		if (el.innerText === "Dashboards") {
			window.location.href = "http://127.0.0.1:5500";
		}
		if (el.innerText === "Pages") {
			window.location.pathname = "pages-login.html";
		}
		if (el.innerText === "Account") {
			window.location.pathname = "account-overview.html";
		}
		if (el.innerText === "Widgets") {
			window.location.pathname = "widgets-stats.html";
		}
		if (el.innerText === "Utilities") {
			window.location.pathname = "horizontol-search.html";
		}
		if (el.innerText === "UI Components") {
			window.location.pathname = "components-buttons.html";
		}
		if (el.innerText === "Applications") {
			window.location.pathname = "project-management.html";
		}
	});
});
...
links.forEach((link) => {
	if (link.attributes.href.value == location.pathname) {
		...
		link.children[0].classList.add("text-blue-700", "bg-blue-50");
		// console.log(
